...
export default function PropertyDescriptionStep() {
  ...
  const handleChange = (e) => setDescription(e.target.value.slice(0, 400));
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const res = await axios.patch(
        `${API_BASE}/api/property/${propertyData.propertyId}/description`,
        body,
        { headers }
      );

      const updatedDescription = res?.data?.property?.description || description;

      setPropertyData((prev) => ({
        ...prev,
        description: updatedDescription,
        stepsCompleted: { ...(prev.stepsCompleted || {}), description: true },
      }));
      ...
      const msg =
        ...
        "Erreur lors de l'enregistrement de la description.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
